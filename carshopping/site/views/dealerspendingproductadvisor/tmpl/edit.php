...
<form class="surveys" action="<?php echo JRoute::_('index.php?option=com_carshopping&task=dealersproductadvisor.approve'); ?>" method="post" name="adminForm" id="adminForm">
	<fieldset>
		<legend>Pending Product advisor profile: </legend>		
		...
		<div class="control-group">
			<div class="control-label">				
				<label id="dealer-lbl" for="approved" class="hasTooltip required" title="Approved">Approved</label>																				
			</div>
			<div class="controls">
				<input type="hidden" name="productadvisor_id" value="<?php echo $this->profile->created_by;?>" />
				<input type="checkbox" name="approved" id="approved" <?php if($this->profile->block==0)echo 'checked'; ?> />
			</div>
		</div>
	</fieldset>
