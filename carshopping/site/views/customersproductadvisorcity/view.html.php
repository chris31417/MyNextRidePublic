 ...
 */
class CarshoppingViewCustomersproductadvisorcity extends JViewLegacy {
    ...
    public function display($tpl = null) {					
        ...
        if(!$user->guest){				
			...
			$optcities = array();
			$optcities[] = JHTML::_('select.option', 0, "Select a city");
			foreach($cities as $city){
				$optcities[] = JHTML::_('select.option', $city->id, $city->name);
			}
			$this->ddlcities = $optcities;
			$this->productAdvisors = null;
			$this->city_id = 0;
			if($city_id!=0){
				$this->city_id = $city_id;
				$this->productAdvisors = $model->getAllProductAdvisorsByCityId($city_id);
