...
class mosVehicleManager extends JTable
{
    ...
    {
        $this->_db->setQuery("SELECT id FROM #__vehiclemanager_rent WHERE fk_vehicleid='$this->vehicleid' " . $exclusion . " ORDER BY id");
        if (version_compare(JVERSION, '3.0', 'lt'))
        {
            $tmp = $this->_db->loadResultArray();
        } else
    ...
    {
        $this->_db->setQuery("SELECT id FROM #__vehiclemanager_rent_request WHERE fk_vehicleid='$this->id'" . $exclusion . " ORDER BY id");
        $tmp = $this->_db->loadResultArray();
        $retVal = array();
        for ($i = 0, $j = count($tmp); $i < $j; $i++) {
    ...
    {
        $this->_db->setQuery("SELECT id FROM #__vehiclemanager_buying_request WHERE fk_vehicleid='$this->id'" . $exclusion . " ORDER BY id");
        $tmp = $this->_db->loadResultArray();
        $retVal = array();
        for ($i = 0, $j = count($tmp); $i < $j; $i++) {
    ...
    {
        $this->_db->setQuery("SELECT thumbnail_img, main_img FROM #__vehiclemanager_photos WHERE fk_vehicleid='$this->id'" . $exclusion . " ORDER BY id");
